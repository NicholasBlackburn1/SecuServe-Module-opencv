...
class RequiredCode(object):
    ...
    def setupPipeline(self, sender):
        ...
        self.sendProgramStatus(sender, "SETUP_PIPELINE", "Starting to run pipleline",
                               imports.datetime.now()-pipeline_start_setup)
        ...
        imports.consoleLog.Warning(
            "is opencv opdemised"+str(imports.cv2.useOptimized()))
        ...
        imports.consoleLog.PipeLine_Ok(
            "STAGE COMPLETE"+str(imports.datetime.now() - pipeline_start_setup))
        # updates stats message
        self.sendProgramStatus(sender, "SETUP_PIPELINE", "finishes setup pipeline to run",
                               imports.datetime.now()-pipeline_start_setup)
        return
    ...
    def trainPipeLine(self, sender):
        ...
        # updates stats message
        self.sendProgramStatus(sender, "TRAIN_MODEL", "starting  to train model",
                               imports.datetime.now() - pipeline_train_knn)

        imports.consoleLog.Warning(
            "Training Model Going to take a while UwU..... ")
        knnClasifiyer.train(train_dir=const.imagePathusers,
                            model_save_path=const.Modelpath, n_neighbors=2)

        imports.consoleLog.PipeLine_Ok(
            "Done Train Knn pipeline timer" + str(imports.datetime.now() - pipeline_train_knn))
        imports.consoleLog.Warning("Done Training Model.....")
        self.sendProgramStatus(sender, "STAGE_COMPLETE", "done training model",
                               imports.datetime.now() - pipeline_train_knn)
        return

    def reconitionPipeline(self, sender):

        self.sendProgramStatus(sender, "SETUP_PIPELINE",
                               "Starting Face rec", imports.datetime.now())
        # cleans mess as we keep prosessing
        imports.gc.collect()

        # Camera Stream gst setup
        gst_str = str(const.opencvconfig['Stream_intro'] +
                      const.opencvconfig['Stream_ip']+":"+const.opencvconfig['Stream_port'])

        imports.consoleLog.Warning("Looking for Faces...")

        imports.consoleLog.Error(
            "User list size is  "+" " + str(len(const.userList)))
        ...
        while True:
            ...
            if(const.watchdog == 10):
                ...
                self.sendProgramStatus(
                    sender, "ERROR", "WATCHDOG OVERRRAN", imports.datetime.now())
                break
                        ...
                        if(name != None):
                            ...
                            if name not in const.userList[self.i]:
                                ...
                                if(status == Status.UNKNOWN):
                                    ...
                                    self.sendUserInfoToSocket(
                                        sender=sender, status="Unknown`", user=name, image=const.unknown_pic_url, time=imports.datetime.now(), phonenumber=4123891615)
                                    imports.consoleLog.PipeLine_Ok("stop face prossesing timer unknown" + str(
                                        imports.datetime.now()-face_processing_pipeline_timer))

                                    self.sendFaceCount(
                                        sender, self.Total, self.Unreconized, self.Reconized, imports.datetime.now())
                            ...
                            if name in const.userList[self.i]:
                                ...
                                if phone == None or 0000000000 or 0:
                                    phone = int(
                                        const.phoneconfig['default_num'])

                                if (status == Status.ADMIN):
                                    self.sendUserInfoToSocket(
                                        sender=sender, status=status, user=usrname, image=const.admin_pic_url, time=imports.datetime.now(), phonenumber=phone)
                                   # imports.logging.info("got an Admin The name is"+str(usrname))
                                    userstat.userAdmin(self=userstat, status="Admin", name=str(usrname), frame=frame, font=font, imagename=imports.datetime.now().strftime(
                                        "%Y_%m_%d-%I_%M_%S_%f"), imagepath=const.imagePath, left=left, right=right, bottom=bottom, top=top, recperesntage=const.facepredict)
                                    imports.consoleLog.PipeLine_Ok(
                                        const.StopingMess+"admin" + str(imports.datetime.now()-face_processing_pipeline_timer))

                                if (status == Status.USER):
                                    self.sendUserInfoToSocket(
                                        sender=sender, status=status, user=usrname, image=const.user_pic_url, time=imports.datetime.now(), phonenumber=phone)
                                    imports.logging.info(
                                        "got an User Human The name is"+str(usrname))
                                    userstat.userUser(self=userstat, status="User", name=usrname, frame=frame, font=font, imagename=imports.datetime.now().strftime(
                                        "%Y_%m_%d-%I_%M_%S_%f"), imagepath=const.imagePath, left=left, right=right, bottom=bottom, top=top, recperesntage=const.facepredict)
                                    imports.consoleLog.Warning(
                                        "eeeep there is an User They Might be evil so um let them in"+"  `"+"There Name is:" + str(usrname))
                                    imports.consoleLog.PipeLine_Ok(
                                        const.StopingMess + "user" + str(imports.datetime.now()-face_processing_pipeline_timer))

                                if (status == Status.UNWANTED):
                                    self.sendUserInfoToSocket(
                                        sender=sender, status=status, user=usrname, image=const.unwanted_pic_url, time=imports.datetime.now(), phonenumber=phone)
                                    imports.logging.info(
                                ...
                                if(self.getAmmountOfFaces(frame) > 2):
                                    self.sendUserInfoToSocket(
                                        sender=sender, status="Group", user=name, image=const.group_pic_url, time=imports.datetime.now(), phonenumber=4123891615)
                                    userStats.UserStats.userGroup(self=userstat, frame=frame, font=font, imagename=imports.datetime.now().strftime(
                                        "%Y_%m_%d-%I_%M_%S_%f"), imagepath=const.imagePath, left=left, right=right, bottom=bottom, top=top)
                                    imports.consoleLog.PipeLine_Ok(
                                        const.StopingMess + "Group" + str(imports.datetime.now()-face_processing_pipeline_timer))

                        else:

                            imports.consoleLog.PipeLine_Ok("Time For non Face processed frames" + str(
                                imports.datetime.now()-face_processing_pipeline_timer))
...
# Encodes all the Nessiscary User info into Json String so it can be easly moved arround
    ...
    def UserDataList(self):
        ...
        while True:
            ...
            userinfo = imports.userData.UserData(imports.mydb.getName(imports.mydb.getFaces(), i), imports.mydb.getStatus(imports.mydb.getFaces(), i), imports.mydb.getImageName(
                imports.mydb.getFaces(), i), imports.mydb.getImageUrI(imports.mydb.getFaces(), i), imports.mydb.getPhoneNum(imports.mydb.getFaces(), i))
            local_data = {
    ...
    def downloadUserFaces(self, imagePath):
        ...
        while True:
            ...
            imports.consoleLog.PipeLine_Data(
                "downloaded"+" "+str(index+1) + " out of " + str(imports.mydb.getAmountOfEntrys()) + "\n")
    ...
    def sendFaceCount(self, sender, total, unknown, reconized, time):
        ...
        sender.send_json({"total": int(total), "unknown": int(
            unknown), "reconized": str(reconized), "time": str(time)})
        imports.time.sleep(.5)
    ...
    def sendUserInfoToSocket(self, sender, status, user, image, time, phonenumber):
        ...
        sender.send_json({"usr": str(user), "status": str(status), "image": str(
            image), "phone": str(phonenumber), "Accuracy": str(const.facepredict), "time": str(time)})
        imports.time.sleep(.5)
